...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        # Use a 3x3 Sobel operator to compute x and y derivatives
        ...
        # Apply a Gaussian filter with a 5x5 kernel and 0.5 sigma
        Ixx = scipy.ndimage.gaussian_filter(Ixx, sigma=0.5)
        Iyy = scipy.ndimage.gaussian_filter(Iyy, sigma=0.5)
        Ixy = scipy.ndimage.gaussian_filter(Ixy, sigma=0.5)

        # Compute the Harris response for each pixel
        k = 0.1  # Sensitivity factor, specified in the document
        detM = Ixx * Iyy - Ixy ** 2
        traceM = Ixx + Iyy
    ...
    def detectKeypoints(self, image):
        ...
        for y in range(height):
            for x in range(width):
                ...
                f.pt = (x, y)
                f.size = 10 # Size of the feature
                f.angle = orientationImage[y, x] # Orientation of the feature
                f.response = harrisImage[y, x] # Harris score of the feature
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            # Handle the case where the window goes out of bounds by adding padding if necessary
            if y-2 < 0 or y+3 > grayImage.shape[0] or x-2 < 0 or x+3 > grayImage.shape[1]:
                patch = np.zeros((5, 5))
            else:
                patch = grayImage[y-2:y+3, x-2:x+3]

            desc[i] = patch.flatten() if patch.size == 25 else np.zeros(25)
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            trans_mx1 = transformations.get_trans_mx(np.array([-f.pt[0], -f.pt[1], 0]))

            # Rotate to align with the x-axis
            rotate_mx = transformations.get_rot_mx(0,0, -np.radians(f.angle))

            # scale down by 5. 40x40 -> 8x8
            scale_mx = transformations.get_scale_mx(0.2, 0.2, 1)

            # Translate to the center of the 8x8 window
            trans_mx2 = transformations.get_trans_mx(np.array([4, 4, 0]))

            transMx = np.dot(trans_mx2, np.dot(scale_mx, np.dot(rotate_mx,trans_mx1)))[:2,(0,1,3)]
            ...
            # if the variance is negligibly small, set the descriptor vector to zero
            if std < 1e-5:
                ...
                for outcol in range(4):
                    for inrow in range(4):
                        for incol in range(4):
                            distcol = outcol*4 + incol # Calculate the column index for the sub-region
                            distrow = outrow*4 + inrow # Calculate the row index for the sub-region
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        # Compute the SSD distance between each pair of features (desc1, desc2)
        ...
        for i, ssd in enumerate(distance):
            ...
            match.trainIdx = np.argmin(ssd)
            match.distance = ssd[np.argmin(ssd)]
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        distance = scipy.spatial.distance.cdist(desc1, desc2)

        for i, ssd in enumerate(distance):
            match = cv2.DMatch()
            match.queryIdx = i

            best_match_index = np.argpartition(ssd,1)[:2]
            best_match_dist = ssd[best_match_index]

            match.distance = best_match_dist[0]**2/best_match_dist[1]**2
            match.trainIdx = best_match_index[0]
